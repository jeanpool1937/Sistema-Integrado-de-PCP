...
export const FilterBar: React.FC<FilterBarProps> = ({
    ...
    const grupos = Array.from(
        new Set(
            skus
                .filter(s => !selectedJerarquia || s.jerarquia1 === selectedJerarquia)
                .map(s => s.grupoArticulosDesc)
                .filter(Boolean)
        )
    ).sort();

    const activeCount = (selectedJerarquia ? 1 : 0) + (selectedGrupo ? 1 : 0);
    ...
    return (
        <div className="flex flex-wrap items-center gap-3 bg-dark-900 border border-slate-800 rounded-xl px-4 py-3 mb-6">
            ...
            {activeCount > 0 && (
                <button
                    onClick={() => { setSelectedJerarquia(''); setSelectedGrupo(''); }}
